...
def cleanup_old_access_logs():
    """
    3개월 이상 된 접속 로그 삭제
    개인정보 처리방침: 접속 로그 3개월 보관
    """
    try:
        from models import AccessLog
        AccessLog.cleanup_old(days=90)
        logger.info("[Cleanup] 3개월 초과 접속 로그 삭제 완료")
    except Exception as e:
        logger.error(f"[Cleanup] 접속 로그 삭제 실패: {e}")
